...
func TestParser_Parse_EscapeSequences(t *testing.T) {
	...
	parser, err := NewParser(bytes.NewReader([]byte(testString)), WithEscapeSequences())
	if err != nil {
		t.Fatalf("NewParser(): %v", err)
	}
...
func TestParser_Parse_Error_EmptyInput(t *testing.T) {
	t.Parallel()

	parser, err := NewParser(bytes.NewReader([]byte("")))
	if err != nil {
		t.Fatalf("NewParser(): %v", err)
	}
	...
	_, err = parser.Parse()
	if err == nil {
		t.Fatal("Parse() succeeded, expected EOF error for empty input")
	}

	if err != io.EOF {
		t.Errorf("got error %v, expected io.EOF", err)
	}
}
...
func TestParser_Parse_Error_InvalidTokenAtRoot(t *testing.T) {
	...
	wantErr := fmt.Sprintf("invalid token %q for root key", LBRACE)
	if err.Error() != wantErr {
		t.Errorf("got error %q, expected %q", err, wantErr)
	}
}
	...
	testString := `"root"
{
	...
	wantErr := fmt.Sprintf("invalid token %q, expected STRING, IDENTIFIER, or WHITESPACE", LBRACE)
	if err.Error() != wantErr {
		t.Errorf("got error %q, expected %q", err, wantErr)
	}
}
...
func TestNewParser_Options(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			opts: []Option{WithEscapeSequences()},
		},
	}
